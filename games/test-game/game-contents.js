...
var sprites = allGameData.sprites.map(datum => new Sprite(datum[0], spriteFiles[datum[1]], datum[2]))
var sounds = allGameData.sounds.map(datum => new Sound(datum[0], datum[1], audioFiles[datum[2]]))


var music = {}
allGameData.music.forEach(datum => { music[datum[1]] = new Sound(datum[0], datum[1], audioFiles[datum[2]]) })


var characterModels = {}
for (let key in allGameData.characterModels) {
    let datum = allGameData.characterModels[key]
    characterModels[key] = new Character.Model(datum[0], datum[1])
}

var worldItemModels = {}
for (let key in allGameData.worldItemModels) {
    let datum = allGameData.worldItemModels[key]
    worldItemModels[key] = new WorldItem.Model(datum[0], datum[1])
}


var makeCharacters = function()  {
    return allGameData.characters.map(datum => new Character(
...
var makeRooms = function()  {
    ...
    return allGameData.rooms.map(datum => 
        {
...
var makeInventoryItems = function() {
    ...
    return  allGameData.inventoryItems.map(datum =>  new InventoryItem(
        datum[0],
