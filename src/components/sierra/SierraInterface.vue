<template>
  <aside class="sierra">
    <div
      class="sierra__menu-wrapper"
      v-bind:class="{
        disabled:(gameStatus !== 'LIVE'),
    }"
    >
      <Tile
        v-bind:icon="verbIcon"
        v-bind:background="verbBackground"
        backgroundColor="white"
        v-bind:isStatic="true"
...
export default {
  ...
  computed: {
    verbIcon() {
      ...
      if (selectedInventoryItem && currentVerb.usesSelectedItem) {
        return selectedInventoryItem.rightPicture;
      }
    ...
    verbBackground() {
      const { currentVerb, selectedInventoryItem } = this;
      if (selectedInventoryItem && currentVerb.usesSelectedItem) {
        return selectedInventoryItem.backgroundStyleObject;
      }
      return false;
    },
    inventoryVerbs() {
      return this.$store.state.gameData.verbList.filter(
        (verb) => verb.showOnInventoryBox
      );
    },
  },
  ...
  mounted() {
    ...
    this.$store.state.gameEmitter.on(
